...
import ShowAllProductsSkeleton from "./ShowAllProductsSkeleton";
...
const ShowAllProducts = () => {
  ...
  if (loading) {
    return <ShowAllProductsSkeleton />;
  }
  ...
  return (
    <div>
      ...
      {productData?.products.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6 mt-7">
          {productData.products.map((product) => (
            <ProductCard
              key={product._id}
              product={product}
              onView={() => navigate(`/admin/products/${product._id}`)}
        ...
        <div className="flex flex-col items-center justify-center text-center py-20 bg-amazon-surface rounded-3xl mx-auto">
          <FaBoxOpen className="text-5xl text-amazon-textLight/40 mb-3" />
          <p className="text-lg font-semibold text-amazon-textLight">
            No products found
          </p>
          <p className="text-sm text-amazon-textLight mt-1">
