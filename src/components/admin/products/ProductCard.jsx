    ...
    <div className="rounded-4xl hover:shadow-xl transition-all duration-200 border border-amazon-border bg-amazon-surface w-full overflow-hidden">
      <div className="relative group overflow-hidden">
        <Swiper
          ...
          pagination={
            paginationEl ? { clickable: true, el: paginationEl } : false
          }
        ...
        <span
          className={`absolute bottom-3 right-3 text-xs font-bold px-3 py-2 rounded-full z-10 backdrop-blur-sm
      ...
      <div className="pb-4 overflow-hidden w-9/10 mx-auto">
        ...
        <div className="flex gap-2">
          {product.tags?.map((tag) => (
            <div
              key={tag}
              className="text-xs font-medium text-white rounded-lg bg-amazon-lightNavy p-1.5"
            >
              {tag}
            </div>
          ))}
        ...
        <div className="text-xs font-medium text-amazon-textDark flex flex-wrap gap-2">
          <button
            className="border border-amazon-border bg-amazon-bg/60 rounded-xl py-2 px-3 shrink-0 whitespace-nowrap cursor-pointer hover:bg-amazon-bg
          ...
          <button
            ...
            onClick={() => onQuickEdit(product._id)}
          >
            <TbAdjustmentsHorizontal /> Quick Edit
          </button>
        </div>
        <div className="flex justify-end mt-4">
          <button
            className="text-xs font-medium text-red-500 border border-red-300 bg-red-200 rounded-xl
